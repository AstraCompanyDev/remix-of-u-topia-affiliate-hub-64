      ...
      <section className="container mx-auto px-6 py-12 md:py-16">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
          ...
          <div className="flex gap-2">
            <Button variant="outline" className="gap-2 text-yellow-600 border-yellow-600/30 hover:bg-yellow-600/10">
              <Ban className="w-4 h-4" />
              Suspend Rewards
            </Button>
            <Button variant="outline" className="gap-2 text-red-600 border-red-600/30 hover:bg-red-600/10">
      ...
      <section className="container mx-auto px-6 pb-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="feature-card p-6">
            ...
            <p className="text-lg font-semibold text-foreground">{format(user.signupDate, 'MMM d, yyyy')}</p>
          </div>
          <div className="feature-card p-6">
            <div className="flex items-center gap-3 mb-3">
              {user.isVerified ? <CheckCircle className="w-5 h-5 text-green-500" /> : <XCircle className="w-5 h-5 text-yellow-500" />}
          ...
          <div className="feature-card p-6">
            ...
            <p className="text-lg font-semibold text-foreground">{format(user.lastActive, 'MMM d, HH:mm')}</p>
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="feature-card p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-lg font-semibold text-foreground">Referral Stats</h3>
                <p className="text-sm text-muted-foreground">User's referral network</p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-4 mb-6">
              ...
              <div className="text-center p-4 bg-muted/30 rounded-lg">
                <p className="text-2xl font-bold text-green-500">{user.convertedReferrals}</p>
                <p className="text-xs text-muted-foreground">Converted</p>
              </div>
              <div className="text-center p-4 bg-muted/30 rounded-lg">
                <p className="text-2xl font-bold text-yellow-500">{user.activeReferrals}</p>
            ...
            <div className="space-y-2">
              {user.referrals.slice(0, 3).map((ref) => (
                <div key={ref.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                  ...
                  <Badge variant="outline" className={ref.status === 'verified' ? 'bg-green-500/10 text-green-500 border-0' : 'bg-yellow-500/10 text-yellow-500 border-0'}>
                    {ref.status}
                  </Badge>
                </div>
              ))}
          ...
          <div className="feature-card p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-lg font-semibold text-foreground">Rewards Breakdown</h3>
                <p className="text-sm text-muted-foreground">Earnings by category</p>
              </div>
            </div>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
                ...
                <span className="text-lg font-semibold text-primary">${user.commissionEarned}</span>
              </div>
              <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
                <div className="flex items-center gap-3">
                  <Gift className="w-5 h-5 text-primary" />
                ...
                <span className="text-lg font-semibold text-primary">${user.bonusesEarned}</span>
              </div>
              <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
                <div className="flex items-center gap-3">
                ...
                <span className="text-lg font-semibold text-primary">${user.dividendsEarned}</span>
              </div>
              <div className="flex items-center justify-between p-4 bg-yellow-500/10 rounded-lg">
                ...
                <span className="text-lg font-semibold text-yellow-500">${user.rewardsPending}</span>
              </div>
            </div>
            <div className="mt-4 pt-4 border-t border-border">
              <div className="flex items-center justify-between">
                <span className="font-medium text-foreground">Total Earned</span>
                <span className="text-xl font-bold text-primary">${user.rewardsEarned}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="feature-card p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{user.purchases.length} purchases</p>
              </div>
            </div>
            <div className="space-y-3">
              {user.purchases.map((purchase) => (
                <div key={purchase.id} className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
                  <div>
                    <p className="text-sm font-medium text-foreground">{purchase.tier} Package</p>
                    <p className="text-xs text-muted-foreground">{format(purchase.date, 'MMM d, yyyy')}</p>
                  </div>
                  <span className="text-lg font-semibold text-primary">${purchase.amount}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="feature-card p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{user.payouts.length} payouts</p>
              </div>
            </div>
            <div className="space-y-3">
              {user.payouts.map((payout) => (
                <div key={payout.id} className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{format(payout.date, 'MMM d, yyyy')}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className="bg-green-500/10 text-green-500 border-0">
                      {payout.status}
                    </Badge>
                    <span className="text-lg font-semibold text-primary">${payout.amount}</span>
                  </div>
                </div>
              ))}
            </div>
